...
1. <del>If your requirement is to have strong encryption using the keystore and losing the encrypted data is a no-no - targeting L+ (5+) seems like the only choice, using `.setEncryptionRequired()`. However be warned that this is no guarentee that it will always work as this seems to still be broken for non-primary user accounts.</del> This is no longer true due to the regression in M-6-23. Option 2 below is the best choice (of a bad bunch)
